...
class Dataloader:
    ...
    def _get_iter(self, start_idx=0):
        B = None#初始B矩阵
        C = None
        for i in range(start_idx, len(self.ds)):#从头开始加载每个时刻的边信息
            if B is None:#如果B为空
                t, dt, B, delta_B, C, delta_C, users, items, _feats = self.ds.getitem(i, False)#求出i时刻的B，

            else:
                B += delta_B
                C += delta_C
                t, dt, _, delta_B, _, delta_C, users, items, _feats = self.ds.getitem(i, True)
            adj = biadjacency_to_laplacian(C,B) * self.alpha
...
class Dataset:
    ...
    def getitem(self, idx, only_delta=True):
        t = self.unique_ts[idx]#找到第idx时刻的时间戳
        dt = t - (self.unique_ts[idx-1] if idx > 0 else self.t0)#找到idx时刻的时间偏移
        a = self.cum_n_records[idx]#找到前idx时刻的交互记录数
        b = self.cum_n_records[idx+1]#找到前idx+1时刻的交互记录数
        if only_delta:
            observed_mat = None
            observed_mat2 = None
        else:
        ...
        delta_mat = self.build_ui_mat(self.df.iloc[a:b])#构造a到b时刻之间的边矩阵
        delta_mat2 = self.build_uu_mat(self.df.iloc[a:b])#构造a到b时刻之间的边矩阵
        users = self.df.iloc[a:b, 0].values#找到a到b时刻受影响的user
        items = self.df.iloc[a:b, 1].values#找到a到b时刻受影响的item
        feats = self.features[a:b]#废话
